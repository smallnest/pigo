...
统一 Provider 抽象：从 `Provider` 接口经两种驱动（OpenAI 兼容 / Anthropic）汇入共享传输层，SSE 解码后以事件流回传，双失败模型贯穿始终。

![pigo 架构总览](book/images/fig4-7.svg)

> 更多分层图解（事件骨架、工具批量执行、子 Agent 委派等）见配套电子书。
