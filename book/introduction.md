...
先读第1章打地基，搞清 pigo 怎么从一条命令启动、把各个组件装配成一个可运行的程序。接着看第2章的 `agentcore` 抽象——那是贯穿全书每一站的类型契约（消息、内容、事件、工具、钩子）。然后精读第3章的两层 Agent 循环——这是全书的心脏，LLM、上下文与工具正是在这里被串成整体；理解了它，其余章节都是对它的展开。

再往下是接入与执行：第4章讲 Provider 层，即如何统一对接不同的大模型服务；第5章讲工具系统，即 LLM 的"手脚"怎么被定义、调用与约束。第6章的上下文压缩和第7章的会话持久化，回答"如何在有限窗口里记住足够多、并把对话存下来"。第8章聚焦项目信任与安全——一个能执行命令的 Agent，怎么才能不闯祸。第9章的子 Agent（sub-agent）机制，展示怎么用 Agent 调度 Agent 来拆解复杂任务。最后第10章讲可扩展性生态，把 pigo 放回更大的工具版图里审视。

如果你时间有限、只想抓精髓，那么第1章和第3章无论如何都不该跳过：它们分别交代了"程序怎么搭起来"和"Agent 怎么转起来"。

不过在钻进任何一章之前，我强烈建议你先读紧接引言之后的那篇**《主线导读：一条命令的生命周期》**。它用一条真实命令，把上面这十章串成一条完整的调用链——十个包各自在什么时候被唤醒、又把接力棒交给了谁。有了这张全书地图，你再进入每一章，都能清楚自己正站在主线的哪一站，读起来就不会有"东一榔头西一棒槌"的散乱感。

好了，磨刀已毕。让我们先看那条主线，再从第1章开始，看 pigo 是如何把一条命令，变成一个会思考、会动手的 AI Agent 的。
